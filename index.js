...
class JSZM {
  ...
  *genPrint(text) {
  ...
  get(x) {
    return this.view.getInt16(x, this.littleEndian);
  }
  ...
  getu(x) {
    return this.view.getUint16(x, this.littleEndian);
  }
  ...
  handleInput(str, t1, t2) {
    ...
    w = x => (
      (i = 0),
      x.split('').filter(y => (i += /[a-z]/.test(y) ? 1 : /[0-9.,!?_#'"\/\\:\-()]/.test(y) ? 2 : 4) < 7).join('')
    );
    br = JSON.parse(
      '[' +
        str
          .replace(
            this.regBreak,
            (m, o) => ',[' + m.length + ',' + (this.vocabulary.get(w(m)) || 0) + ',' + (o + 1) + ']'
          )
          .slice(1) +
        ']'
    );
  ...
  put(x, y) {
    return this.view.setInt16(x, y, this.littleEndian);
  }
  putu(x, y) {
    return this.view.setUint16(x, y & 65535, this.littleEndian);
  }
